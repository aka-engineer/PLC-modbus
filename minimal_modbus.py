...
while True:
    client.write_bit(RUN_POSITION, 0)
    time.sleep(0.5)
    client.write_bit(RUN_POSITION, 1)
    time.sleep(0.5)
    client.write_long(SET_POSITION_SERVO3, 1000000, byteorder=minimalmodbus.BYTEORDER_LITTLE_SWAP)
    data = client.read_long(SET_POSITION_SERVO3, byteorder=minimalmodbus.BYTEORDER_LITTLE_SWAP)
    print(data)
    time.sleep(5)
    client.write_bit(RUN_POSITION, 0)
    time.sleep(0.5)
    client.write_bit(RUN_POSITION, 1)
    time.sleep(0.5)
    client.write_long(SET_POSITION_SERVO3, 0, byteorder=minimalmodbus.BYTEORDER_LITTLE_SWAP)
    data = client.read_long(SET_POSITION_SERVO3, byteorder=minimalmodbus.BYTEORDER_LITTLE_SWAP)
    print(data)
    time.sleep(5)
